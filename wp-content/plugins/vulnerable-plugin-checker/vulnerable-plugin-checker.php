	...
	class Vulnerable_Plugin_Checker {
			...
			if ( isset( $installed_plugins[ $file_path ]['Version'] ) ) {
				...
				if ( isset( $plugin['vulnerabilities'] ) && is_array( $plugin['vulnerabilities'] ) ) {
					...
					foreach ( $plugin['vulnerabilities'] as $vulnerability ) {
						...
						$vul_fixed_in = $this->normalize_version( $vulnerability['fixed_in'] );
						$plugin_version = $this->normalize_version( $plugin['Version'] );

						if ( null == $vulnerability['fixed_in'] || version_compare( $vul_fixed_in, $plugin_version ) > 0 ) {			
		...
		public function after_row_text( $plugin_file, $plugin_data, $status ) {
			...
			foreach ( $vulnerabilities['vulnerabilities'] as $vulnerability ) {
				...
				$plugin['is_known_vulnerable'] = 'false';
				$vul_fixed_in = $this->normalize_version( $vulnerability['fixed_in'] );
				$plugin_version = $this->normalize_version( $plugin_data['Version'] );
		...
		/**
		 * Normalize Version
		 * makes all version formats to 0.0.0
		 * @param  string $version
		 * @return string          normalized version
		 */
		public function normalize_version( $version ) {
			while ( 2 > substr_count( $version, '.' ) ) {
				$version .= '.0';
			}
